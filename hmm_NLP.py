...
def learn_model(
    ...
    for key in obs_dict.keys():
        obs_dict[key] = np.divide(
            obs_dict[key],
            obs_dict_totals,
            out=np.zeros_like(obs_dict[key]),
            where=obs_dict_totals != 0,
        )
    for jj in range(17):
        transition_prob[:, jj] = np.divide(
            transition_prob[:, jj],
            sum(transition_prob[:, jj]),
            out=np.zeros_like(transition_prob[:, jj]),
            where=transition_prob[:, jj] != 0,
        )
...
def viterbi_forward(
    X0: npt.NDArray, Tprob: npt.NDArray, Oprob: Dict[str, npt.NDArray], obs: List[str]
...
def viterbi_backward(m: npt.NDArray, pointers: npt.NDArray) -> List[str]:
    most_likely_path = []
...
def backward(
    Tprob: npt.NDArray, Oprob: Dict[str, npt.NDArray], obs: List[str], k: int
) -> npt.NDArray:
    b = np.ones(len(Tprob))
    obs = obs[k + 1 :]
    obs.reverse()
...
def expected_emissions(
    ...
    for w in expected_emissions_dict.keys():
        np.divide(
            expected_emissions_dict[w],
            expected_emissions_totals,
            out=expected_emissions_dict[w],
            where=expected_emissions_totals != 0,
        )
